# Notes

## Run Flask App Locally
- `source .venv/Scripts/Activate`
- make sure `which pip` is in the right folder for activated environment if installing requirements with `pip install -r requirements.txt`
- run app with `python app.py`

## Install Pip Packages
- export TMPDIR=/home/ec2-user/pip_cache
- pip install -r requirements.txt --no-cache-dir

## Misc
- `aws s3 cp s3://michael-models/fasterrcnn_resnet50_fpn.pth .`
- show ec2 cpu usage - `mpstat`
- show processes `ps`

## Legacy EC2 / Nginx Notes (DEAD CODE)
These instructions were for running Flask via gunicorn directly on EC2 and managing nginx manually. They are no longer used — Docker containers now run the app, and CDK manages infrastructure. See [cdk/README.md](../cdk/README.md) for current deployment info.

- Old: `nohup gunicorn -w 1 -b 0.0.0.0:5000 app:app > gunicorn_output.log 2>&1 &`
- Nginx commands (now inside container):
  - `sudo systemctl start nginx`
  - `sudo systemctl status nginx`
  - `sudo systemctl restart nginx`
